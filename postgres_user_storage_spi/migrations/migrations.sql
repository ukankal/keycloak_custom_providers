...
CREATE TABLE credential (id UUID DEFAULT gen_random_uuid() NOT NULL, device CHARACTER VARYING(255), hash_iterations INTEGER DEFAULT 27500 NOT Null, salt BYTEA, type CHARACTER VARYING(255), value CHARACTER VARYING(4000), user_id UUID, created_date TIMESTAMP(6) WITHOUT TIME ZONE DEFAULT now(), counter INTEGER DEFAULT 0, digits INTEGER DEFAULT 6, period INTEGER DEFAULT 30, algorithm CHARACTER VARYING(36), CONSTRAINT pk_credential PRIMARY KEY (id), CONSTRAINT fk_credential_user FOREIGN KEY (user_id) REFERENCES "user_entity" ("id") ON DELETE CASCADE ON UPDATE CASCADE);
